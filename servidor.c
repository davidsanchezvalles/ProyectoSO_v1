	...
	while (terminar==0){
		...
		switch (codigo)
		{
			...
			case 10: {	//inicio de partida
				...
				//10/nombre/vides_que_tinc/vides_que_te/força/graus
				...
				if(vides1 == 0){
					...
					strcat(consulta,"',");
					strcat(consulta,"100");//La duracion?? s'ha de posar? podria contarho el client i enviar el temps quan algu guanyi(potser serà més fàcil, en c# hi ha funcions per a tot)
					strcat(consulta,"',");
					strcat(consulta, nombre);
					strcat(consulta,"');");
					...
					strcat(consulta, "1");
					strcat(consulta,"',");
					strcat(consulta,"10");//poso puntuació de 10 al que guanya
					strcat(consulta,"',");
